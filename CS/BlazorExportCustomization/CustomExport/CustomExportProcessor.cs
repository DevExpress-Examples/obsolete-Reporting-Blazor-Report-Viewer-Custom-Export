...
namespace BlazorExportCustomization.CustomExport {
    public class CustomExportProcessor : IExportProcessor {
        ...
        public CustomExportProcessor(CustomExportStorage exportStorage, IWebHostEnvironment env, IHttpContextAccessor httpContextAccessor, IAntiforgery antiforgery) {
        ...
        public Task<ExportResultRequestData> GetExportInfoAsync(ExportResultItem item) {
            return Task.FromResult(new ExportResultRequestData() {
                ...
                FormData = new Dictionary<string, string>() {
                    ["Id"] = item.Id,
                    ["__RequestVerificationToken"] = _antiforgery.GetAndStoreTokens(_httpContextAccessor.HttpContext).RequestToken
                }
            });
        }
    }
